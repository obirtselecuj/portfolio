...
const ResumeCard = ({ item: { title, description, subTitle, result, course, address, school } }) => {
  return (
    <div className='w-full h-1/3 group flex'>
      ...
      <div className='w-full  bg-black bg-opacity-20 hover:bg-opacity-30 duration-300 rounded-lg p-4 lgl:px-10 flex flex-col justify-center gap-3 lgl:gap-3 shadow-shadowOne'>
        <div className='flex flex-col lgl:flex-row justify-between gap-4 lgl:gap-0 lgl:items-center'>
          <div>
            <h3 className='text-xl md:text-2xl font-semibold group-hover:text-white duration-300'>{title}</h3>
            <h3 className='text-xl md:text-2xl font-semibold group-hover:text-white duration-300'>{course}</h3>
